#!/usr/bin/env pwsh
# Script de vérification du système wallet après migration

Write-Host "`n🔍 VÉRIFICATION DU SYSTÈME WALLET" -ForegroundColor Cyan
...
Write-Host "📊 VÉRIFICATION DES FICHIERS LOCAUX`n" -ForegroundColor $info
...
# 4. Vérifier les composants frontend modifiés
Write-Host "`n📱 VÉRIFICATION DES COMPOSANTS FRONTEND`n" -ForegroundColor $info
...
Write-Host "`n📚 VÉRIFICATION DE LA DOCUMENTATION`n" -ForegroundColor $info
...
# 6. Rechercher problèmes potentiels
Write-Host "`n🔍 RECHERCHE DE PROBLÈMES POTENTIELS`n" -ForegroundColor $info
...
Test-Check "No deprecated walletService imports" {
    ...
    if ($deprecated_imports.Count -gt 0) {
        Write-Host "`n   ⚠️  Files still importing walletService: $($deprecated_imports -join ', ')" -ForegroundColor $warning
    }
    $deprecated_imports.Count -eq 0
}

# 7. Vérifier Git status
Write-Host "`n🔄 VÉRIFICATION GIT`n" -ForegroundColor $info
...
if ($checks_failed -eq 0) {
    ...
    Write-Host "`nℹ️  Prochaines etapes:" -ForegroundColor $info
    ...
    Write-Host "⚠️  CERTAINES VERIFICATIONS ONT ECHOUE" -ForegroundColor $warning
