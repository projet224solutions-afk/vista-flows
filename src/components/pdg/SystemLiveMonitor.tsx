    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-[95vw] max-h-[95vh] w-full md:w-[1400px] h-[95vh] md:h-[850px] p-0 overflow-hidden bg-black border-0" aria-describedby={undefined}>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 gap-3 border-b border-emerald-500/30 bg-black/80 backdrop-blur">
            <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 10, repeat: Infinity, ease: 'linear' }}
              >
                <Shield className="w-8 h-8 sm:w-10 sm:h-10 text-emerald-400 flex-shrink-0" />
              </motion.div>
              <div className="min-w-0 flex-1">
                <h1 className="text-base sm:text-2xl font-bold text-emerald-400 font-mono tracking-wider truncate">
                  SYSTÈME DE SURVEILLANCE 24/7
                </h1>
                <p className="text-emerald-500/70 text-xs sm:text-sm font-mono truncate">
            ...
            <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto justify-between sm:justify-end">
              ...
              <Button
                ...
                size="icon"
                onClick={() => refetch()}
                className="text-emerald-400 hover:text-white hover:bg-emerald-500/20 h-8 w-8 sm:h-10 sm:w-10"
                disabled={loading}
              >
          ...
          <div className="relative flex-1 p-3 sm:p-6 overflow-y-auto overflow-x-hidden" ref={containerRef}>
            ...
            <div className="relative z-10 flex gap-3 sm:grid sm:grid-cols-4 sm:gap-4 mb-6 sm:mb-8 overflow-x-auto pb-2 sm:pb-0 scrollbar-hide -mx-3 px-3 sm:mx-0 sm:px-0">
              ...
              <div className="relative flex-shrink-0 w-[140px] sm:w-auto">
                <motion.div
                  ...
                  className="p-3 sm:p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/30 backdrop-blur cursor-pointer hover:bg-emerald-500/20 transition-colors h-full"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 sm:gap-2 text-emerald-500/70 text-[10px] sm:text-xs mb-1.5 sm:mb-2">
                      <CreditCard className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                      <span className="truncate">TRANSACTIONS (AUJOURD'HUI)</span>
                    </div>
                    <ChevronDown className={`w-3 h-3 sm:w-4 sm:h-4 text-emerald-500/70 transition-transform flex-shrink-0 ${showTransactionMenu ? 'rotate-180' : ''}`} />
                  </div>
                      ...
                      >
                        <div className="p-2 sm:p-3 space-y-1">
                          ...
                          <div 
                            ...
                            onClick={() => setShowTransactionMenu(false)}
                          >
                            <span className="text-emerald-400 text-xs sm:text-sm font-medium">📊 Année</span>
              ...
              {[
                ...
                <motion.div
                  ...
                  className="p-3 sm:p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/30 backdrop-blur flex-shrink-0 w-[120px] sm:w-auto"
                >
                  <div className="flex items-center gap-1.5 sm:gap-2 text-emerald-500/70 text-[10px] sm:text-xs mb-1.5 sm:mb-2">
            ...
            <div className="relative z-10 flex justify-center">
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="inline-flex flex-col sm:flex-row items-center gap-3 sm:gap-8 p-3 sm:p-6 rounded-xl sm:rounded-2xl bg-gradient-to-r from-emerald-500/10 via-emerald-500/20 to-emerald-500/10 border border-emerald-500/30 w-full sm:w-auto"
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  >
                    <RefreshCw className="w-5 h-5 sm:w-8 sm:h-8 text-emerald-400" />
                  </motion.div>
                ...
                <div className="flex items-center gap-2 sm:gap-3 font-mono text-2xl sm:text-4xl text-emerald-400 font-bold">
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-[8px] sm:text-xs text-emerald-500/50">JOURS</span>
                  </div>
                  <span className="text-emerald-500/50">:</span>
                  <div className="flex flex-col items-center">
                    <span>{String(uptime.hours).padStart(2, '0')}</span>
                    <span className="text-[8px] sm:text-xs text-emerald-500/50">HEURES</span>
                  </div>
                  <span className="text-emerald-500/50">:</span>
                  <div className="flex flex-col items-center">
                    <span>{String(uptime.mins).padStart(2, '0')}</span>
                    <span className="text-[8px] sm:text-xs text-emerald-500/50">MINS</span>
                  </div>
                  <span className="text-emerald-500/50">:</span>
                  <div className="flex flex-col items-center">
                    <motion.span
                    ...
                    <span className="text-[8px] sm:text-xs text-emerald-500/50">SECS</span>
                  </div>
                </div>

                <div className="flex items-center gap-1.5 sm:gap-2">
                  <CheckCircle2 className="w-4 h-4 sm:w-6 sm:h-6 text-emerald-400" />
                  <span className="text-emerald-400 font-bold text-xs sm:text-base">OPÉRATIONNEL</span>
                </div>
              </motion.div>
            </div>
          </div>
          ...
          <div className="relative z-10 flex items-center justify-between p-2 sm:p-4 border-t border-emerald-500/30 bg-black/80 backdrop-blur gap-2">
            <div className="flex items-center gap-3 sm:gap-6 text-[10px] sm:text-xs font-mono text-emerald-500/70 overflow-x-auto scrollbar-hide flex-1">
