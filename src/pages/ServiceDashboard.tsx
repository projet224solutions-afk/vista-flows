...
// Types de services qui ont leur propre module complet (pas besoin du dashboard générique)
const FULL_MODULE_TYPES = ['location', 'immobilier'];
...
function isFullModuleService(service: ProfessionalService): boolean {
  ...
  return (
    FULL_MODULE_TYPES.includes(code) ||
...
export default function ServiceDashboard() {
  ...
  const { DollarSign, TrendingUp, Users } = require('lucide-react');
  const { BookingManagement } = require('@/components/professional-services/modules/BookingManagement');
...
// Composant séparé pour le dashboard générique
function GenericServiceDashboard({ service }: { service: ProfessionalService }) {
  // Lazy imports to avoid loading for real estate
  const { Card, CardContent, CardHeader, CardTitle } = require('@/components/ui/card');
  const { Tabs, TabsContent, TabsList, TabsTrigger } = require('@/components/ui/tabs');
  const { DollarSign, TrendingUp, Users } = require('lucide-react');
  const BookingManagement = require('@/components/professional-services/modules/BookingManagement').BookingManagement;
