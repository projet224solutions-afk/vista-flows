...
const userTypes = {
  ...
  admin: {
    label: "PDG",
    description: "Pr├®sident Directeur G├®n├®ral",
    color: "bg-primary"
  },
  pdg: {
    label: "PDG",
...
export default function Profil() {
  ...
  // R├®cup├®rer le type d'agent dynamiquement si l'utilisateur est un agent
  useEffect(() => {
    const fetchAgentType = async () => {
      if (profile?.role === 'agent' && user?.id) {
        try {
          ...
          console.error('Erreur r├®cup├®ration type agent:', err);
        }
      }
    };
    fetchAgentType();
  ...
  const handleSaveSettings = async () => {
    ...
    setSaving(true);
    try {
      // Mise ├á jour de l'email
      if (editEmail !== user.email) {
        const { error: emailError } = await supabase.auth.updateUser({
          email: editEmail
        });
        if (emailError) throw emailError;
        toast.success('Email mis ├á jour. V├®rifiez votre bo├«te mail pour confirmer.');
      }

      // Mise ├á jour du t├®l├®phone dans le profil
      if (editPhone !== profile?.phone) {
        ...
        toast.success('Num├®ro de t├®l├®phone mis ├á jour');
      }
      ...
      toast.error(error?.message || 'Erreur lors de la mise ├á jour');
    } finally {
      setSaving(false);
    }
  };
  ...
  const onAvatarClick = async () => {
    ...
    input.onchange = async () => {
      ...
      if (!file || !user) return;
      try {
        // Upload vers GCS via le hook unifi├®
  ...
  const handleGoBack = async () => {
    ...
    if (profile?.role === 'vendor_agent') {
      // R├®cup├®rer le token d'acc├¿s et rediriger vers l'interface agent
          ...
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-6">
                Vous devez vous connecter pour acc├®der ├á votre profil
              </p>
              <Button
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="space-y-2">
              ...
              <p className="text-[10px] sm:text-xs text-muted-foreground">
                ­ƒÆí Partagez cet ID pour permettre ├á d'autres utilisateurs de vous contacter directement
              </p>
            </div>
          </CardContent>
        </Card>
