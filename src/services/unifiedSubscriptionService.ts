  ...
  static async getSubscriptionStats(): Promise<SubscriptionStats | null> {
    try {
      ...
      const [
        ...
        supabase.from('subscriptions').select('s:*, p:plans!inner(user_role)', { count: 'exact', head: true }).eq('status', 'active').eq('plans.user_role', 'vendeur'),
        supabase.from('subscriptions').select('s:*, p:plans!inner(user_role)', { count: 'exact', head: true }).eq('status', 'active').eq('plans.user_role', 'taxi'),
        supabase.from('subscriptions').select('s:*, p:plans!inner(user_role)', { count: 'exact', head: true }).eq('status', 'active').eq('plans.user_role', 'livreur')
      ]);
