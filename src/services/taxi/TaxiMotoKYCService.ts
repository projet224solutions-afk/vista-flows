...
export class TaxiMotoKYCService {
  ...
  static async getKYCStatus(driverId: string): Promise<KYCStatus> {
    try {
      ...
      const allVerified = documents?.every(d => d.status === 'approved') || false;
      const hasRejected = documents?.some(d => d.status === 'rejected') || false;
  ...
  static async verifyDocument(
    ...
    try {
      ...
      if (approved) {
        ...
        if (doc) {
          const kycStatus = await this.getKYCStatus(doc.driver_id);
          if (kycStatus.can_work) {
            await supabase
              .from('taxi_drivers')
              .update({ kyc_verified: true, can_work: true })
              .eq('id', doc.driver_id);
          }
        }
      }
