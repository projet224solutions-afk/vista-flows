...
async function run() {
    ...
    const sql = await readFile('./sql/security_unified_hardening.sql', 'utf8');
    const client = buildClient(url);
    await client.connect();
    try {
        await client.query(sql);
        console.log('✅ Unified security hardening appliqué.');
    } catch (e) {
        console.error('[ERROR] Unified security SQL failed:', e?.message || e);
        process.exit(1);
    } finally {
        await client.end();
    }
