...
serve(async (req) => {
  ...
  try {
    ...
    // Pour simplifier, on renvoie les credentials
    // En production, vous devriez générer un vrai token RTC avec la bibliothèque Agora
    return new Response(
      JSON.stringify({
        appId: AGORA_APP_ID,
        token: null, // Token temporaire pour le développement
        channel,
        uid,
        role
      }),
