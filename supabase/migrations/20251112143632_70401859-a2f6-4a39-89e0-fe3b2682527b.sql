-- Drop old function versions and create the correct one
-- First drop all versions of create_escrow_transaction
DROP FUNCTION IF EXISTS public.create_escrow_transaction(UUID, UUID, UUID, DECIMAL, TEXT, JSONB);
DROP FUNCTION IF EXISTS public.create_escrow_transaction(UUID, UUID, DECIMAL, UUID, TEXT, JSONB);

-- Now create the correct version with proper parameter mapping
