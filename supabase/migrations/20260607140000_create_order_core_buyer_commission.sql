-- 💰 COMMISSION ACHETEUR + COMMISSION VENDEUR CENTRALISÉE — create_order_core
--
-- Décisions PDG (2026-06-07) :
--   1. COMMISSION ACHETEUR (system_settings.purchase_fee_percent) = prélevée EN PLUS sur l'acheteur
--      et GARDÉE par la plateforme. Avant : calculée seulement dans le frontend ProductPaymentModal
--      → jamais débitée pour les paiements wallet. Désormais débitée atomiquement ici et créditée
--      au wallet PDG (pdg_management.user_id), dans la devise du wallet acheteur.
--   2. COMMISSION VENDEUR = passée par le backend (PLATFORM_FEE_RATES : physique 5%, digital 10%,
--      service 7%) et STOCKÉE sur escrow.commission_amount → utilisée telle quelle à la libération
--      (confirm_delivery_and_release_escrow lit commission_amount en priorité).
--
-- Nouveaux paramètres (DEFAULT → rétro-compatibles) :
--   p_buyer_fee_amount         : commission acheteur, devise wallet acheteur (débitée EN PLUS, → PDG)
--   p_seller_commission_amount : commission vendeur, devise commande (stockée sur l'escrow)
...
BEGIN
  ...
  v_total_debit    := p_wallet_debit_amount + v_buyer_fee;   -- acheteur débité = produit + commission acheteur
  ...
  -- ── PHASE 0 : Vérification solde (produit + commission acheteur) ──
  ...
  -- ── PHASE 5 : Escrow (commission_amount = commission VENDEUR, utilisée à la libération) ──
  ...
  -- ── PHASE 6 : Débit wallet acheteur (produit + commission acheteur) ──
  IF p_payment_method='wallet' AND p_buyer_user_id IS NOT NULL AND v_total_debit>0 THEN
    ...
    -- COMMISSION ACHETEUR → wallet plateforme (PDG), dans la devise du wallet acheteur.
    IF v_buyer_fee > 0 THEN
      ...
      IF v_pdg_user_id IS NOT NULL THEN
        UPDATE public.wallets SET balance = balance + v_buyer_fee, updated_at = now()
        WHERE user_id = v_pdg_user_id AND currency = v_wallet_cur;
        IF NOT FOUND THEN
          INSERT INTO public.wallets (user_id, balance, currency, wallet_status)
          VALUES (v_pdg_user_id, v_buyer_fee, v_wallet_cur, 'active');
        END IF;
        INSERT INTO public.wallet_transactions (transaction_id, sender_user_id, receiver_user_id,
          transaction_type, amount, net_amount, description, status, metadata)
        VALUES ('fee-' || left(replace(gen_random_uuid()::text,'-',''),45), p_buyer_user_id, v_pdg_user_id,
          'commission', v_buyer_fee, v_buyer_fee, 'Commission acheteur marketplace', 'completed',
          jsonb_build_object('order_id', v_order_id, 'wallet_currency', v_wallet_cur, 'source', 'buyer_commission'));
      END IF;
    END IF;
  END IF;
...
SELECT 'create_order_core: commission acheteur (→PDG) + commission vendeur (escrow.commission_amount) ajoutées.' AS status;
