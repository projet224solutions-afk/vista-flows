-- =========================================
-- SYSTÈME AUTOMATIQUE DE CRÉATION DE LIVRAISONS (CORRIGÉ)
...
BEGIN
  ...
  IF NEW.status IN ('confirmed', 'preparing') AND 
    ...
    -- Récupérer l'adresse du vendeur (type TEXT)
    SELECT address INTO vendor_addr
    FROM vendors v
    ...
    SELECT
      ...
      CASE 
        WHEN vendor_addr IS NOT NULL AND vendor_addr != '' 
        THEN jsonb_build_object('address', vendor_addr)
    ...
    RAISE NOTICE 'Livraison créée pour la commande %', NEW.order_number;
  END IF;
