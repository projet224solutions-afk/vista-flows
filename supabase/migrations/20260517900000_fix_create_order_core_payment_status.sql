-- ================================================================
-- FIX create_order_core — payment_status correct pour wallet (2026-05-17)
--
-- Problème :
--   create_order_core crée toujours la commande avec payment_status='pending'.
--   Quand Payment.tsx bypasse le backend (create_marketplace_order_secure),
--   il n'y a aucun orders.update pour corriger le statut → commande reste
--   'pending' même si le wallet a été débité.
--
-- Fix :
--   - wallet + p_wallet_debit_amount > 0 → 'paid' (wallet débité dans Phase 6)
--   - cash → 'pending' (paiement à la livraison)
--   - card / mobile_money → 'pending' (vérifié en dehors par le backend)
--   - wallet + p_wallet_debit_amount = 0 → 'pending' (pas de débit → backend gère)
--
...
SELECT 'create_order_core — payment_status correct pour wallet (paid quand débité).' AS status;
