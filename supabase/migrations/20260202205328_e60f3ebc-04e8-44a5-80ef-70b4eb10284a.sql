-- Corriger la fonction create_online_order pour éviter la double décrémentation du stock
-- Le trigger decrement_stock_on_order_items gère déjà la décrémentation lors de l'insertion dans order_items
...
BEGIN
  ...
  -- Le trigger 'decrement_stock_on_order_items' sur la table order_items
  -- gère automatiquement la décrémentation du stock.
  -- Décrémenter manuellement ici causerait une DOUBLE décrémentation (-2 au lieu de -1).
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    v_product_id := (v_item->>'product_id')::UUID;
    v_requested_quantity := (v_item->>'quantity')::INTEGER;
    
    -- Créer l'item de commande (le trigger gère le stock)
    ...
    -- ❌ SUPPRIMÉ: UPDATE products SET stock_quantity = stock_quantity - v_requested_quantity
    -- Le trigger decrement_stock_on_order_items s'en charge automatiquement
  END LOOP;
