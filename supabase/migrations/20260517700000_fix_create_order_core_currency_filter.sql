-- ================================================================
-- FIX create_order_core — Filtre devise wallet (2026-05-17)
--
-- Problème corrigé :
--   Phase 0 (vérification solde) et Phase 6 (débit wallet) utilisaient
--   WHERE user_id = p_buyer_user_id SANS filtre sur currency.
--
--   Risque : pour un achat cross-currency (acheteur GNF, vendeur XOF),
--   le backend calcule le walletDebitAmount en devise acheteur (GNF)
--   mais la requête SQL pouvait matcher n'importe quel wallet de l'user.
--   Avec le filtre, on garantit qu'on débite uniquement le wallet dans
--   la devise attendue.
--
-- Nouveau paramètre :
--   p_buyer_wallet_currency TEXT DEFAULT NULL
--   Si NULL → comportement identique à avant (rétrocompatible)
--   Si fourni → filtre WHERE currency = p_buyer_wallet_currency
-- ================================================================
...
BEGIN
  ...
  -- p_wallet_debit_amount = montant DÉJÀ converti en v_wallet_cur par le backend
  IF p_payment_method = 'wallet'
     AND p_buyer_user_id IS NOT NULL
     AND p_wallet_debit_amount > 0
  THEN
...
SELECT 'create_order_core — filtre devise wallet opérationnel.' AS status;
