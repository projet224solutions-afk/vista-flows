-- ================================================================
-- FIX record_service_subscription_payment (2026-05-17)
--
-- Problèmes corrigés :
--   1. INSERT wallet_transactions utilisait wallet_id et type
--      qui n'existent pas → colonnes correctes : sender_user_id,
--      transaction_type (enum), transaction_id (NOT NULL UNIQUE),
--      net_amount, currency
--   2. p_price_paid INTEGER → NUMERIC (wallets utilisent DECIMAL)
--   3. Vérification solde wallet sans filtre devise
--   4. Débit wallet sans filtre devise
--   5. transaction_id manquant (contrainte UNIQUE NOT NULL)
-- ================================================================

-- Supprimer l'ancienne signature INTEGER (CREATE OR REPLACE ne remplace
-- que si la signature est identique — le changement INTEGER→NUMERIC crée
-- un doublon qui déclenche "could not choose best candidate function")
DROP FUNCTION IF EXISTS public.record_service_subscription_payment(uuid, uuid, uuid, integer, text, text, text);
...
SELECT 'record_service_subscription_payment — fix wallet_transactions colonnes + devise + NUMERIC.' AS status;
