-- ================================================================
-- MIGRATION : Corriger create_order_core (version 14 paramètres)
--
-- Bugs dans 20260518950000 :
--   1. Phase 5 — buyer_id = p_customer_id (customers.id ≠ auth.users.id)
--      → violates FK escrow_transactions_buyer_id_fkey
--      Fix : buyer_id = COALESCE(p_buyer_user_id, p_customer_id)
--
--   2. Phase 2 — payment_status hardcodé 'pending' même pour wallet
--      → wallet orders apparaissent comme impayées
--      Fix : v_payment_status calculé (paid si wallet+débit>0)
-- ================================================================
...
BEGIN
  ...
  INSERT INTO escrow_transactions (
    ...
    v_order_id,
    v_escrow_buyer,      -- FIX: COALESCE(p_buyer_user_id, p_customer_id) → auth.users UUID
    p_vendor_user_id,
    p_buyer_user_id,
    p_vendor_user_id,
...
SELECT 'create_order_core — buyer_id FK corrigé (v_escrow_buyer) + v_payment_status wallet.' AS status;
