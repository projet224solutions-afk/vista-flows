-- Supprimer la politique problématique héritée
DROP POLICY IF EXISTS "allow_all_authenticated" ON syndicate_workers;

-- Recréer la fonction create avec BYPASSRLS via session_replication_role
...
BEGIN
    ...
    -- Générer un ID personnalisé séquentiel (MBR0001, MBR0002, etc.)
    ...
    -- Insérer le nouveau membre (SECURITY DEFINER bypass RLS car owner = postgres)
    INSERT INTO public.syndicate_workers (
        ...
        p_telephone,
        COALESCE(p_access_level, 'member'),
...
-- Ajouter une politique pour permettre INSERT/UPDATE/DELETE via le rôle postgres
CREATE POLICY "Allow postgres role full access"
