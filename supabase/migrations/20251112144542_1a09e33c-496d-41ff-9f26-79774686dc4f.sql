-- Fix create_escrow_transaction function to use correct column names in escrow_logs
-- Drop all existing versions of the function
DROP FUNCTION IF EXISTS public.create_escrow_transaction(uuid, uuid, uuid, numeric, text, jsonb);
DROP FUNCTION IF EXISTS public.create_escrow_transaction(uuid, uuid, numeric, text, jsonb);

-- Recreate the function with correct column names (action and note instead of action_type and notes)
...
BEGIN
  ...
  -- Log the escrow creation using correct column names: action and note
