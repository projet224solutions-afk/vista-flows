-- ─────────────────────────────────────────────────────────────────
-- Fix admin_change_vendor_currency: met aussi à jour wallets.currency
-- Quand le PDG change la devise d'un vendeur, le wallet doit refléter
-- la nouvelle devise immédiatement (sinon l'interface vendeur reste incohérente)
-- ─────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- 2. Récupérer la devise actuelle et l'user_id du vendeur
  SELECT shop_currency, user_id INTO v_old_currency, v_vendor_user_id
  FROM vendors
  ...
  -- 6. Mettre à jour le wallet du vendeur (devise + verrouillage)
  UPDATE wallets
  SET
    currency         = UPPER(p_new_currency),
    currency_locked  = true,
    updated_at       = now()
  WHERE user_id = v_vendor_user_id;

  -- 7. Flaguer les produits actifs pour révision des prix
  UPDATE products
  SET
