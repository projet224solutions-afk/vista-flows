-- =============================================================================
-- FIX: trigger_surveillance_order_total - Ajouter rule_id obligatoire
-- =============================================================================
...
DECLARE
  ...
  v_rule_id TEXT := 'ORD_004'; -- Rule ID pour les anomalies de total de commande
BEGIN
  ...
  IF NEW.total_amount IS NOT NULL AND ABS(NEW.total_amount - v_calculated_total) > 0.01 THEN
    -- Vérifier si la règle existe, sinon la créer
    INSERT INTO public.logic_rules (rule_id, name, domain, rule_type, severity, is_active)
    VALUES (v_rule_id, 'Vérification total commande', 'ORDERS', 'validation', 'HIGH', true)
