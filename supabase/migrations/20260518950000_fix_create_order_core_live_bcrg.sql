-- ================================================================
-- MIGRATION : create_order_core — Vérification last_bcrg_scraped_at
--
-- But : Phase -1 vérifie maintenant que le taux GNF provient d'un
--   vrai scraping du site bcrg-guinee.org, pas d'un simple heartbeat.
--
-- Règle ajoutée :
--   Pour toute transaction impliquant GNF avec source_type='official_html' :
--     • last_bcrg_scraped_at doit exister (scraping réel enregistré)
--     • last_bcrg_scraped_at doit être < 24h
--   Si non → transaction REFUSÉE avec message explicite.
--
--   Distinction heartbeat vs scraping :
--     retrieved_at        : mis à jour toutes les heures (heartbeat ou scrape)
--     last_bcrg_scraped_at: mis à jour UNIQUEMENT lors d'un scrape HTML réussi
-- ================================================================
...
BEGIN
  ...
  --
  -- Pour GNF official_html : last_bcrg_scraped_at doit exister et être < 24h
  --   (distingue un heartbeat d'un vrai scraping du site bcrg-guinee.org)
  ...
  THEN
    ...
    -- Vérification BCRG obligatoire pour GNF : refus fallback_api
    ...
    -- Vérification scraping réel BCRG (last_bcrg_scraped_at) pour GNF
    IF (p_currency = 'GNF' OR v_wallet_cur = 'GNF')
...
SELECT 'create_order_core — last_bcrg_scraped_at vérifié pour GNF (< 24h).' AS status;
